...
def get_binance_price(symbol: str) -> float | None:
    """Binance REST API로 현재가 조회 (실패 시 None 반환)"""
    try:
        resp = requests.get(
...
def bsol_live(df: pd.DataFrame) -> None:
    ...
    px      = get_binance_price("SOLUSDT") or float(latest["implied_sol_px"])
    ...
    chart.loc[chart.index[-1], "implied_sol_px"] = px   # 마지막 점만 실시간 교체
...
def ibit_live(df: pd.DataFrame) -> None:
    ...
    px      = get_binance_price("BTCUSDT") or float(latest["implied_btc_px"])
    gap_pct = (px - avg_cost) / avg_cost * 100
    ...
    chart.loc[chart.index[-1], "implied_btc_px"] = px   # 마지막 점만 실시간 교체
    chart = chart.set_index("date")
    chart.columns = ["시장가 (Market Price)", "기관 평단가 (Cost Basis)"]
...
ic2.success("⚡ 현재 시장가: 바이낸스 실시간 조회 · 10초마다 자동 갱신")
...
with tab1:
    ...
    if not df_bsol.empty:
        ...
        flow["color"] = flow["flow_sol_final"].apply(
...
with tab2:
    ...
    if not df_ibit.empty:
        ...
        flow["color"] = flow["flow_btc_final"].apply(
            lambda x: "#2ecc71" if x >= 0 else "#e74c3c"
        )
